...
#| warning: false
#| message: false
#| title: "Critical Values of the t-distribution (Two-Tailed Test)"
...
# kable(ttab, 
#       caption = "Critical Values of the t-distribution (Two-Tailed Test)") |> 
...
## Appendix: Critical Values for Chi2 ($\chi^2$) Distribution

The table below gives the critical values ​​of the chi-square distribution ($\chi^2$) at a confidence level of 95% (i.e., $\alpha = 0.05$), for various degrees of freedom (df).

```{r}
#| echo: false
#| warning: false
#| message: false
#| title: "Critical Values of Chi2 Distribution (95% Level)"

library(gt)

chisqtab <- data.frame(
  df = 1:30,
  critical_value = round(qchisq(0.95, df = 1:30), 3)
)

names(chisqtab) <- c("df", "Critical Value (α = 0.05)")
