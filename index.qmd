...
1.  **Data Cleaning:** This part focuses on the steps researcher's often need to take in order to prepare their data for analysis ("data cleaning"), including the creation of factor variables
2.  **Linear Statistical Models**: This part focuses on examining variation in continuous dependent variables using a linear statistical model with a particular focus on linear regression.
3.  **Logistic Regression**: This part focuses on predicting binary dependent variables using a logistic regression model.
4.  **Interactions within Linear and Logistic Models**: This part focuses on incorporating interaction terms into linear and logistic regression models to examine whether the effect of one independent variable depends on the properties of another independent variable.

These sections introduce the syntax used to perform the core methods of analysis in the class. We will break down the contents of this syntax and explain its logic when the different commands that we will use. In addition, we will oftentimes annotate the syntax via comments (either presented in grayed out text or in bubbles that you can examine by hovering over them with your computer mouse) to further call your attention to aspects of the syntax. Here is an example:
...
For each week in the course, you need to read relevant chapters ....

| Week | Section | Chapters |
|------------------------|------------------------|------------------------|
| 1 | Linear Models | @sec-investigate-data; @sec-linear-covcor; @sec-reporting-and-presenting-results (10.2-10.4) |
| 2 | Linear Models | @sec-factor-command; @sec-binary-predictors; @sec-statistical-significance ; @sec-predicted-residual-values (7.1 & 7.2) ; @sec-reporting-and-presenting-results (10.4) |
| 3 | Linear Models | @sec-multiple-linear-regression ; @sec-predicted-residual-values (7.3) ; @sec-model-fit ; @sec-reporting-and-presenting-results (10.5 - 10.7) |
