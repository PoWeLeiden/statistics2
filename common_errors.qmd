...
::: {.callout-warning title="The Problem"}
We're using `avg_slopes()` to try and find the slope for a variable, or `predictions()` to find predicted values, but are running into an error: "Unable to compute predicted values with this model"
:::
