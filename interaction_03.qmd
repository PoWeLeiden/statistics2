...
#Data
anes <- import("anes_interactions.rda")
...
If we want to plot the results using the `linetype` argument, then we need to first convert the `socialists` variable in the predictions dataset that we just created into a factor variable.
