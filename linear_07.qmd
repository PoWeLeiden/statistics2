...
model_multiple <- lm(v2x_polyarchy ~ gini_2019 + TYPEDEMO1984_factor + 
                       GDP2006, data = demdata)
...
The coefficients for `gini_2019` (p = 0.322) and `TYPEDEMO1984_factor` (p = 0.246) are statistically insignificant. Is that because of too much multicollinearity? We can again rely on the `car` package to answer this question via its `vif()` function.
...
vif_example <- lm(v2x_polyarchy ~ gini_2019 + GDP2006 + 
                    Typeregime2006_factor, data = demdata) 
...
resid_panel(bivar_model, plots = c("resid"), smoother = FALSE)
```

`resid_panel(bivar_model`
: We use the `resid_panel()` function and provide to it the name of the model we're interested in examining. 

`plots = c("resid")`
: This function can be used to produce various types of plots (as you'll see below). Here, we clarify that we want a plot showing residuals against predicted values ("resid"). 

`smoother = FAlSE)`
: The most up-to-date version of this package will show a non-parametric regression line like the one seen in @sec-quadratic-relationships in this type of plot by default. We use this syntax to turn that option off because this type of line can oftentimes create more confusion than clarification when trying to interpret patterns in this type of plot. 
...
```{r}
resid_panel(model_multiple, plots = c("resid"), smoother = FALSE)
```
...
We are only interested in partial regression plots for continuous predictors. We can thus ignore the plot for `TYPEDEMO1984_factor`.
...
multiple_ln <- lm(v2x_polyarchy ~ gini_2019 + TYPEDEMO1984_factor + 
                    LNGDP2006, data=demdata)

#Residual Plot
resid_panel(multiple_ln, plots = c("resid"), smoother = FALSE)
...
The interpretation of a logged predictor variable changes how we interpret the coefficient for that variable:

-   A 1% increase in X (on its original scale) leads to a change in Y of coefficient value \* log(1.01) units in Y
-   A 10% increase in X (on its original scale) leasd to a change in Y of coefficient value \* log(1.10) units in Y

We advise making a figure with predicted values to investigate the relationship between a logged predictor variable and a dependent variable to help clarify the nature of the relationship.

We would first examine the distribution of our logged variable for the observations in our model (i.e., after removing observations with an NA value on one or more variables in the model):

```{r}
# First check the values of GDP2006 for the observations used in the model
demdata_sub <- demdata |> 
  filter(complete.cases(v2x_polyarchy, gini_2019, 
                        TYPEDEMO1984_factor, LNGDP2006)) 

# Check the variable's distribution
summary(demdata_sub$LNGDP2006)
```

We would then make predictions across the range of the logged X variable. Here, you should make predictions for many values of X so that a nice curved line can be plotted.

```{r}
# Then, make predictions based on the values you obtained
...
1.  `seq()` stands for sequence. We are asking the command to calculate predictions for the sequence of values starting from 5.5, and ending at 11, in 0.5 increments.

We then create a new variable in the dataset with the predicted values to serve as the X-variable in our plot. Specifically, we exponentiate the logged X variable using the `exp()` command. This *reverses* the logarithim thereby enabling us to plot the predicted values against X on its original scale.

```{r}
# Exponentiation of the logged predictor:
bbp_preds <- bbp_preds |> 
  mutate(GDP2006 = exp(LNGDP2006)) # <1>
```

1.  `exp()` basically reverses the log, i.e., `(x)` = `exp(log(x))`

We finally make our graph:
...
ggplot(bbp_preds, aes(x = GDP2006, y = estimate)) + 
  ...
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), 
              alpha = 0.1) + 
  labs(title = "Predicted Electoral Democracy Score Based on GDP in 2006", 
       x = "GDP in 2006", 
       y = "Predicted Value")
```

Interpretations also change if you log-transform the dependent variable as well. We refer you to the following two sources for further guidance on how to interpret regression models with log-transformed variables: [Source 1](https://kenbenoit.net/assets/courses/ME104/logmodels2.pdf){target="_blank"} and [Source 2](https://library.virginia.edu/data/articles/interpreting-log-transformations-in-a-linear-model){target="_blank"}
:::
...
```{r}
resid_panel(Violence_linmodel, plots = c("resid"), smoother = FALSE)
```

We can check whether a quadratic relationship is a better fit for the data by adding a quadratic term to our regression model. We can do this in either of two ways. First, we can manually create this variable first and add it to our dataset via `mutate()` before including it in our resulting model. Second, we can directly add the transformation to the model via the `I()` command. This is a little simpler since we do not need to create the varaible first and, perhaps more importantly, works better with the commands from `marginaleffects` that we use to interpret models (e.g., `predictions()`).
...
#Alternative method
Violence_sqmodel_alternative <- lm(pve ~ v2x_polyarchy + 
                                     I(v2x_polyarchy^2), data = demdata)

# Model results
tidy(Violence_sqmodel)
tidy(Violence_sqmodel_alternative)
```
...
The inclusion of a quadratic predictor complicates the interpretation of the relationship between X and Y. The following rules apply:

-   If the coefficient for X is positive and the one for X^2^ is negative, then the relationship between X and Y is **concave** (an inverted U shape)
-   If the coefficient for X is negative and the one for X^2^ is positive, then the relationship between X and Y is **convex** (a U-shape)
-   If both coefficients point in the same direction (both positive or both negative), then this implies an effect of X on Y that amplifies or becomes stronger as X increases in value

Much as with exponential relationships, it is often better to clarify the relationship visually. We do this by first identifying the range of X that we want to make predictions with a focus on just those observations that were used in fitting the model (i.e., no NA values on any of the included variables).

```{r}
#First, we check the values of X for the observations used 
...
We then obtain the predicted values via `predictions()` and plot them. It is a good idea to ask for many predicted values here so that a nice curved line can be drawn.

```{r}

# Then we make predictions based on these values
pve_preds <- predictions(Violence_sqmodel_alternative, 
                         newdata = datagrid(v2x_polyarchy= seq(from=0, 
                                                               to=0.9, 
                                                               by=0.05)))
...
ggplot(pve_preds, aes(x=v2x_polyarchy, y=estimate)) + 
  ...
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), 
              alpha = 0.1) + 
  labs(title = "Electoral Democracy and Political Violence",
       x = "Electoral Democracy",   
       y = "Predicted Value") 
```

**Important**: We calculated predicted values from the model where we included the squared term via the `I()` syntax rather than from the model where we first created the squared term in our dataset using `mutate()`. We needed to do this because `predictions()` will not produce accurate predictions in the latter scenario. If we use `predictions()` with that latter model, then `predictions()` would calculate predictions for each value of X while holding X^2^ constant at its mean value; you can see an example of this behavior below. This is problematic: if X = 0.50, then X^2^ *must* equal 0.25 rather than the mean of X^2^ for the correct prediction to be calculated. The use of `I()` avoids this problem and generates the correct predicted values.

```{r}
# Predictions using the squared term we created: y = x + x^2
predictions(Violence_sqmodel, 
                         newdata = datagrid(v2x_polyarchy= seq(from=0, 
                                                               to=0.1, 
                                                               by=0.05))) |> 
  tibble() |> 
  select(rowid, estimate, conf.low, conf.high, v2x_polyarchy_sq, v2x_polyarchy)
```
:::
...
```{r}
resid_panel(Violence_sqmodel, plots = c("resid"), smoother = FALSE)
```
...
```{r}
resid_panel(multiple_ln, plots = c("resid", "hist", "qq"), smoother = FALSE)
```
...
We first investigate the summary statistics of the standardized residuals (saved in the `demdata_multln` dataset) . Specifically, we look at the minimum and maximum values to see any observations cross the different outlier thresholds that we are interested in (\|1.96\|, \|2.58\|, and especially \|3.29\|).
