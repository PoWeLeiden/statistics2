...
package_list <- c("tidyverse", "rio", "summarytools", "DescTools", "skimr",
                  ...
                  "see", "marginaleffects", " broom", "ggResidpanel", "rms",
...
#sessioninfo::session_info(info = "packages", include_base = F)

si <- sessioninfo::session_info(info = "packages", include_base = FALSE)

si$packages <- si$packages[si$packages$attached == TRUE, ] 

si$packages$path <- ""
si$packages$loadedpath <- ""
si$packages$library <- ""


```

While there are a lot of packages listed in the command above, many of them are dependencies of a set of core libraries that we'll use: `tidyverse` (`dplyr` and `ggplot`), `rio`, `modelsummary`, `marginaleffects`, `broom`, `ggResidpanel`, and `car`. The collapsible box below lists the version information for these packages when creating the chapters in this book.
